...
if (isset($_POST['add_books'])) {
  ...
  if (empty($bname)) {
    ...
    $message[] = 'Please Enter Food title';
  } elseif (empty($price)) {
    $message[] = 'Please Enter Food price';
  } elseif (empty($category)) {
    $message[] = 'Please Choose a category';
  } elseif (empty($desc)) {
    $message[] = 'Please Enter Food descriptions';
  } elseif (empty($img)) {
    ...
    if ($add_book) {
      ...
      $message = 'Food Not Added Successfully';
    }
  }
}
...
<head>
  ...
  <link rel="icon" href="images/pizzaboy.png" type="image/x-icon">
...
<a class="update_btn" style="position: fixed ; z-index:100;" href="total_foods.php">See All Food Items</a>
  <div class="container_box">
    <form action="" method="POST" enctype="multipart/form-data">
      <h3>Add&nbsp; Food&nbsp; Items&nbsp; To&nbsp; <a href="index.php"><span>QUICK</span><span>BITE</span></a></h3>
      <input type="text" name="bname" placeholder="Enter Food Name" class="text_field ">
      <input type="text" name="btitle" placeholder="Enter Food name" class="text_field">
      ...
      <option value="New">South Indian</option>
          <!--  <option value="Adventure">Biryani</option>-->
            <option value="Magic">Chicken</option>
            <option value="knowledge">Chinese</option>
         </select>
      <textarea name="bdesc" placeholder="Enter Food description" id="" class="text_field" cols="18" rows="5"></textarea>
      <input type="file" name="image" accept="image/jpg, image/jpeg, image/png" class="text_field">
      <input type="submit" value="Add Food" name="add_books" class="btn text_field">
    </form>
  </div>
...
<form action="" method="post" enctype="multipart/form-data">
   ...
   <select name="update_category" value="<?php echo $fetch_update['category']; ?> required class="text_field">
         <option value="Adventure">South Indian</option>
         <option value="Magic">Chicken</option>
         <option value="knowledge">Chinese</option>
      </select>
  ...
  <section class="show-products">
   ...
   <div class="box-container">
      ...
      <div class="box">
         ...
         <div class="name">Items: <?php echo $fetch_book['title']; ?></div>
         <div class="name">Name: <?php echo $fetch_book['name']; ?></div>
         <div class="price">Price: ₹ <?php echo $fetch_book['price']; ?>/-</div>
         <a href="add_foods.php?update=<?php echo $fetch_book['bid']; ?>" class="update_btn">update</a>
         <a href="add_foods.php?delete=<?php echo $fetch_book['bid']; ?>" class="delete_btn" onclick="return confirm('delete this product?');">delete</a>
      </div>
      <?php
         }
      }else{
...
<section class="edit-product-form">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <input type="hidden" name="update_old_image" value="<?php echo $fetch_update['image']; ?>">
      <img src="./added_books/<?php echo $fetch_update['image']; ?>" alt="">
      <input type="text" name="update_name" value="<?php echo $fetch_update['name']; ?>" class="box" required placeholder="Enter Food Name">
      <input type="text" name="update_title" value="<?php echo $fetch_update['title']; ?>" class="box" required placeholder="Enter Food Name">
      <select name="update_category" value="<?php echo $fetch_update['category']; ?> required class="text_field">
            <option value="New">South Indian</option>
            <option value="Adventure">Biryani</option>
            <option value="Magic">Chicken</option>
